...
UGameSettingCollection* ULyraGameSettingRegistry::InitializeMouseAndKeyboardSettings(ULyraLocalPlayer* InLocalPlayer)
{
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		{
			UGameSettingValueScalarDynamic* Setting = NewObject<UGameSettingValueScalarDynamic>();
			Setting->SetDevName(TEXT("AimCone"));
			Setting->SetDisplayName(LOCTEXT("AimCone_Name", "Aim Cone"));
			Setting->SetDescriptionRichText(LOCTEXT("AimCone_Description", "Change Aim cone."));

			Setting->SetDynamicGetter(GET_SHARED_SETTINGS_FUNCTION_PATH(GetAimConeValue));
			Setting->SetDynamicSetter(GET_SHARED_SETTINGS_FUNCTION_PATH(SetAimConeValue));
			Setting->SetDefaultValue(GetDefault<ULyraSettingsShared>()->GetAimConeValue());
			Setting->SetDisplayFormat(UGameSettingValueScalarDynamic::RawTwoDecimals);
			Setting->SetSourceRangeAndStep(TRange<double>(0, 100), 0.01);
			Setting->SetMinimumLimit(0.01);
